...
const begin = momentLang => {
  window.requirejs(['composer', 'composer/formatting', 'translator'], (composer, formatting, translator) => $(document).ready(() => {
    (0, _translatorModule2.default)(translator.Translator);
    (0, _clientSideTranslation2.default)(translator.Translator);

    ajaxify.loadTemplate('partials/calendar/event-creation-modal', template => translator.translate(template, lang, html => {
      $('body').append(html);
...
try {
  if (momentLang === 'en-us') {
    ...
    require(`bundle-loader!moment/locale/${momentLang}`)(() => {
      // eslint-disable-line
  ...
  try {
    require(`bundle-loader!moment/locale/${momentLang.split('-')[0]}`)(() => {
      // eslint-disable-line
      _moment2.default.locale(momentLang);
      begin(momentLang);
    });
