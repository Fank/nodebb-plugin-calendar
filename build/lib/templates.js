  ...
  <div class="panel-body">
    ...
    if (event.repeats) {
      ...
      if (event.repeats.every.daysOfWeek) {
        const days = event.repeats.every.daysOfWeek.map(day => `[[moment:locale-data, _weekdaysShort, ${day}]]`).join(', ');
        const endDateText = `[[moment:time-date-view, utc, ${event.repeats.endDate}, ` + `${event.repeats.endDate}, true]]`;
        return `<div class="plugin-calendar-event-repeats">
            <i class="fa fa-repeat" aria-hidden="true"></i>
            <span>
