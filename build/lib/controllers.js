...
exports.default = (router, middleware) => {
  ...
  router.get('/api/admin/plugins/calendar/save', (req, res, next) => {
    _bluebird2.default.resolve().then(() => (0, _settings.setSettings)(JSON.parse(req.query.settings))).then(() => res.sendStatus(200)).catch(next);
  });

  router.post('/api/admin/plugins/calendar/add', (req, res, next) => {
    _bluebird2.default.resolve().then(() => (0, _icals.addICal)(req.body)).then(() => res.sendStatus(201)).catch(next);
  });
